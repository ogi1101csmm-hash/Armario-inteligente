...
const CATEGORIES=['Camiseta','Polo','Camisa','Jersey','Sudadera','Chaqueta','Abrigo','Pantalón','Vaquero','Short','Zapatillas','Zapatos','Botas','Accesorio'];
...
function openGarmentForm(g=null){
 ...
 $('#garmentPhoto').value='';$('#garmentDialog').showModal();
}
async function saveGarment(e){
 ...
 const g={id,name:$('#garmentName').value.trim(),category:$('#garmentCategory').value,color:$('#garmentColor').value,style:$('#garmentStyle').value,season:$('#garmentSeason').value,occasions:$$('#garmentOccasions .check-chip.active').map(b=>b.dataset.value),clean:$('#garmentClean').checked,photo,createdAt:existing?.createdAt||new Date().toISOString(),lastWorn:existing?.lastWorn||null,useCount:existing?.useCount||0};
...
function categorySlot(cat){if(['Camiseta','Polo','Camisa','Jersey','Sudadera'].includes(cat))return'top';if(['Pantalón','Vaquero','Short'].includes(cat))return'bottom';if(['Zapatillas','Zapatos','Botas'].includes(cat))return'shoes';if(['Chaqueta','Abrigo'].includes(cat))return'outer';if(cat==='Accesorio')return'accessory';return'other';}
...
function generateOutfit(forceRandom=false){
 ...
 if(items.length<3){toast('Faltan categorías básicas. Añade parte de arriba, pantalón y calzado.');return;}
 // improve colors by retries
 let best=items,bestColor=outfitColorScore(items);for(let r=0;r<24;r++){let trial=required?[required]:[];for(const slot of slots){if(trial.some(x=>categorySlot(x.category)===slot))continue;const p=weightedPick(pools[slot]||[],ctx,trial.map(x=>x.id));if(p)trial.push(p);}if(trial.length>=3){const cs=outfitColorScore(trial);if(cs>bestColor){best=trial;bestColor=cs;}}}
...
function bindEvents(){
 ...
 $('#garmentPhoto').addEventListener('change',async e=>{const f=e.target.files[0];if(!f)return;$('#photoPreview').src=await compressImage(f);$('#photoPreview').classList.remove('hidden');$('#photoPlaceholder').classList.add('hidden');});
